...
\noindent \large \textbf{MAT 130, Handout 27: Prediction intervals for $y$ \& mean of $y$} \large \emph{(Ch 24, 25)}  \normalsize
...
\end{itemize}
Save the script file from today as \texttt{29Nov-Prediction\_intervals.R}
\end{thm}
...
\end{ques}
\begin{solution}
We plug in 1 for \texttt{sexM} when the individual is male, and 0 for \texttt{sexM} when the individual is female.  On average, a male is 5.22 inches taller than a female.
\end{solution}
\vspace{1cm}
...
\item The second of these intervals is called a %\underline{\hspace{3cm}} \underline{\hspace{3cm}}  
...
\end{ques}
\begin{solution}
The \texttt{summary()} function gives the min values.  Then use \texttt{predict()} again to get the intervals.
<<>>=
...
\item CI: (27.3, 31.2)
\item PI: (23.8, 34.8)
\item Compare widths: The intervals are higher and wider than the ones from the previous problem.
\end{itemize}
\end{solution}
...
\end{remark}
\begin{solution}
When the values of the predictor variables move further away from the average the new fitted value becomes skewed.  
\end{solution}
\end{document}
